...
import { convertFromPaise } from "../utils/format-currency";
import { DateRangeEnum, DateRangePreset } from "../enums/date-range.enumm";
...
export const summaryAnalyticsService = async (
  ...
  return {
    availableBalance: convertFromPaise(availableBalance),
    totalIncome: convertFromPaise(totalIncome),
    totalExpenses: convertFromPaise(totalExpenses),
    savingRate: {
    ...
    percentageChange: {
      ...percentageChange,
      previousValues: {
        incomeAmount: convertFromPaise(
          percentageChange.previousValues.incomeAmount
        ),
        expenseAmount: convertFromPaise(
          percentageChange.previousValues.expenseAmount
        ),
        balanceAmount: convertFromPaise(
...
export const chartAnalyticsService = async (
  ...
  const transaformedData = (resultData?.chartData || []).map((item: any) => ({
    date: item.date,
    income: convertFromPaise(item.income),
    expenses: convertFromPaise(item.expenses),
  }));
...
export const expensePieChartBreakdownService = async (
  ...
  const transformedData = {
    totalSpent: convertFromPaise(data.totalSpent),
    breakdown: data.breakdown.map((item: any) => ({
      ...item,
      value: convertFromPaise(item.value),
    })),
  };
